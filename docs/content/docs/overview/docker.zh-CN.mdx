...
主应用是静态前端，浏览器直连外部 AI 或 WebDAV 地址时，目标服务必须允许当前站点的 CORS；否则会出现浏览器跨域错误。仓库自带的 `canvas-proxy` 可作为同源转发服务，Compose 配置已将 `/proxy/` 转发到它，并通过 `CANVAS_PROXY_URL=/proxy` 自动启用：

```bash
docker compose up -d --build
```

代理会把 `/proxy/https://目标地址` 转发到目标地址，只适合部署在你自己的站点并配合 HTTPS 使用。不要把代理单独暴露到公网，也不要把它当作 API Key 的安全边界；密钥仍由浏览器保存并随请求发送。

## 数据说明

当前主应用镜像只启动静态 Nginx，画布、我的素材、生成记录和 AI API Key 默认保存在浏览器本地；代理只负责转发请求，不保存业务数据。
